...
public class TimestampParser
{
  ...
  )
  {
    if (format.equalsIgnoreCase("auto")) {
      ...
      final DateTimes.LocalFormatter parser = DateTimes.wrapFormatterLocal(createAutoParser());
      return (String input) -> {
      ...
      try {
        final DateTimes.LocalFormatter formatter = DateTimes.wrapFormatterLocal(DateTimeFormat.forPattern(format));
        return input -> {
          Preconditions.checkArgument(!Strings.isNullOrEmpty(input), "null timestamp");
